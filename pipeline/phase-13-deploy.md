...
8. **Custom domain connection with canonicalization — a required step, not
   ...
   a. **Ask the client which form should be canonical**: the apex domain
      ...
      form — swap the roles if the client picks `www` instead. (Asking here
      is fine — the site is fully built by this point, and deploy-time domain
      choices are real decisions, not the mid-build check-ins the execution
      model prohibits during Phases 4–12.)
